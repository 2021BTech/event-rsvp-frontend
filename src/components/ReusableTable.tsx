            ...
            <thead className="bg-gray-50">
              <tr>
                {columns.map((col, index) => (
                  <th
                    key={index}
                    className={`px-3 py-3.5 text-left text-sm font-semibold text-gray-900 ${col.className || ""}`}
                  >
                    {col.label}
                  </th>
                ))}
              ...
              {data.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  {columns.map((col, colIndex) => (
                    <td key={colIndex} className="whitespace-nowrap px-3 py-4 text-sm text-gray-700">
